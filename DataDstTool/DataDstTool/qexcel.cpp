...
void QExcel::setCellFontStyle(int row, int cloum, const QString &fontStyle)
{
    QString cell;
    cell.append(QChar(cloum - 1 + 'A'));
    cell.append(QString::number(row));

    QAxObject *range = sheet->querySubObject("Range(const QString&)", cell);
    range = range->querySubObject("Font");
    range->setProperty("Name", fontStyle);
}
...
void QExcel::setCellLineStyle(int row, int column)
{
    QString cell;
    cell.append(QChar(column - 1 + 'A'));
    cell.append(QString::number(row));
    ...
    QAxObject *range = sheet->querySubObject("Range(const QString&)", cell);
    range->setProperty("LineStyle", 1);//xlCenter
